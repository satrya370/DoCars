<footer class="text-center lg:text-left bg-gray-900 text-gray-100">
  <div class="mx-10 lg:px-15 py-10 text-center md:text-left">
    <div class="flex flex-col md:flex-col lg:flex-row mx-auto md:justify-between lg:items-center">
      <div class="basis-1/3 flex flex-col md:px-32 lg:px-0">
        <h6 class="mb-4 font-title text-3xl md:text-center lg:text-left">
          DoCars Bali Tour
        </h6>
        <p class="mt-5 md:text-center lg:text-left">
          My Tour Bali is a reliable driver that can make your holiday in Bali memorable.
        </p>
      </div>
      
      <div class="flex flex-col md:flex-row basis-1/2 lg:justify-between md:justify-around md:mt-10">
        <div class="flex flex-col">
          <div class="mt-5">
            <h6 class="font-subtitle font-semibold mb-4 flex justify-center md:justify-start text-2xl">
              Help
            </h6>
            <p class="mb-4">
              <a href="#!" class="text-cyan-600">How to Booking</a>
            </p>
            <p class="mb-4">
              <a href="#!" class="text-cyan-600">Term and Condition</a>
            </p>
          </div>
          <div class="mt-5">
            <h6 class="font-semibold font-subtitle mb-4 flex justify-center md:justify-start text-2xl">
              Useful Links
            </h6>
            
            <p class="mb-4">
              <a href="/#services" class="text-cyan-600">Services</a>
            </p>
            <p class="mb-4">
              <a href="/#destinations" class="text-cyan-600">Destinations</a>
            </p>
            <p class="mb-4">
              <a href="/#tour-packages" class="text-cyan-600">Tour Packages</a>
            </p>
            <p class="mb-4">
              <a href="/#gallery" class="text-cyan-600">Gallery</a>
            </p>
          </div>
        </div>
        <div class="flex flex-col md:flex-col">
          <div class="mt-5">
            <h6 class="font-subtitle font-semibold mb-4 flex justify-center md:justify-start text-2xl">
              Social Media
            </h6>
            <p class="mb-4">
              <a href="https://instagram.com/balidriverputu?igshid=ZDdkNTZiNTM=" class="text-cyan-600" target="_blank"><i class="icofont-instagram"></i> balidriverputu</a>
            </p>
            <p class="mb-4">
              <a href="https://www.facebook.com/putu.balidriver.3?mibextid=ZbWKwL" class="text-cyan-600" target="_blank">
                <i class="icofont-facebook"></i> Putu Bali Driver</a>
            </p>
            <p class="mb-4 block">
              <a href="https://www.tripadvisor.com/Attraction_Review-g297697-d25288635-Reviews-My_Tour_Bali-Kuta_Kuta_District_Bali.html" class="text-cyan-600 flex justify-center md:justify-start" target="_blank">
                <img src="/icofont/tripadvisor-logo.svg" alt="" class="w-5 mr-1 "> My Tour Bali
              </a>
            </p>
          </div>
          <div class="mt-5">
            <h6 class="font-semibold mb-4 flex justify-center md:justify-start text-2xl">
              Contact
            </h6>
            <p class="mb-4">
              <a href="mailto:darma_eka89@yahoo.co.id" class="">
                <i class="icofont-email"></i> darma_eka89@yahoo.co.id</a>
            </p>
            <p class="mb-4">
              <a href="{{ config('app.whatsapp') }}" class="">
                <i class="icofont-whatsapp"></i> +62-858-8882-1190
              </a>
            </p>
            <p class="mb-4">
              <a href="https://wa.me/6285737691010" class="">
                <i class="icofont-whatsapp"></i> 
                +62-857-3769-1010 (Putu)
              </a>
            </p>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="text-center p-6 bg-gray-900 line border-t-2 border-cyan-100">
    <span>© 2023 Copyright:</span>
    <a class="text-cyan-600 font-semibold" href="/">My Tour Bali</a>
  </div>
</footer>